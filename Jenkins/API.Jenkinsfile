...
def configFromBranch(branch) {
    def env_dev = [
        rancherContext: 'chamilaidec:humiclide',
                ...
                stage ('Deploy') {
                    ...
                    stages{                       
                        ...
                        stage ('Docker - Build') {
                            ...
                            steps {
                                withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', accessKeyVariable: 'somw-aws-id', credentialsId: 'machina-ecr', secretKeyVariable: 'some-aws-example']]) {
                            ...
                            steps {
                                script {
                                    for (deployment in configFromBranch(BRANCH_NAME).deployments) {
                                        sh "echo yes | /usr/bin/rancher login --token token- some-aws-example some-aws-example --context ${deployment.rancherContext} --name rancher2 https://ec2-18-228-89-100.sa-east-1.compute.amazonaws.com/v3"
                                        sh """
                                            sed '
