pipeline {
    ...
    parameters {
        ...
        def url_repo = "http://nexus.macna.local/service/rest/v1/search?" +
             ...
             "&maven.artifactId=ReposicaoMedicamentoFront" +
             "&maven.extension=jar"
    ...
    environment
    {
		OPENSHIFT_PROJECT_NAME = "pbm-reposicao-medicamentos"
        APPLICATION_NAME='reposicaomedicamentobatch'
        REPO_NEXUS="ReposicaoMedicamentoBatch"
		JKS_PRODUCT = "pbm"
        JKS_PROJECT = "pbm-reposicao-medicamentos"
    //http://nexus.macna.local/repository/pbm-mvn/br/com/macna/pbm/reposicaomedicamentos/reposicaomedicamentobase/1.00.00b254/ReposicaoMedicamentoBase-1.00.00b254.jar

        ARTIFACT_DOWNLOAD_URL = "http://nexus.macna.local/repository/pbm-mvn/br/com/macna/pbm/reposicaomedicamentos/${REPO_NEXUS}/${VERSION}/${APPLICATION_NAME}-${VERSION}.jar"
	}
    ...
    stages {
        ...
        stage('Download Artfacts') {
            steps {
                script {
                    sh 'mkdir -p ocp/'
                    sh 'curl -o "ocp/${APPLICATION_NAME}-${VERSION}.jar" "${ARTIFACT_DOWNLOAD_URL}"'
                }
            }
        }
                ...
                script {
                    openshift.withCluster() {                        
                        openshift.withProject("${OPENSHIFT_PROJECT_NAME}-${ENV}") {
                            ...
                            openshift.newBuild("--name=${APPLICATION_NAME}", "--image-stream=openshift/nginx-116", "--binary=true")
                        }
                    }
                }
            }
                        ...
                        openshift.withProject("${OPENSHIFT_PROJECT_NAME}-${ENV}") {
							echo "Create APP : ${APPLICATION_NAME}"
                                if (!openshift.selector('dc', "${APPLICATION_NAME}").exists()) {   
                                    ...
                                    echo "Configmap realms-configmap not found. Let's create!"
