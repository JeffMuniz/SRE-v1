//http://macnaimamara.local:8081/fature/fature-auditado/sync-capeante/blob/master/Jenkinsfile#L497
...
pipeline {
    ...
    environment {
        ...
        PRJ_PATH='.'
        PUBLISH_PATH='app'
        PRJ_NAME='Orizon.SincronizadorCapeante.sln'
        APPLICATION_NAME='fature-auditado-sync-capeante'
        OPENSHIFT_PROJECT_NAME='fature-auditado'
        TEST_PATH='./tests/Orizon.Sincronizador.Capeante.Tests'
        
        //CONFIGMAP_DEV="src/Orizon.Queue.MotorContingencia.Cassi.Worker/appSettings-dev.json"
        //CONFIGMAP_HML="src/Orizon.Queue.MotorContingencia.Cassi.Worker/appSettings-hml.json"
        //CONFIGMAP_PRD="src/Orizon.Queue.MotorContingencia.Cassi.Worker/appSettings-prd.json"
        ...
        SECRET_FILENAME="src/Orizon.Sincronizador.Capeante/appsettings.json"
        
        SECRET_SENHA_DATIVA="senha-dativa"
        
        SONAR_PATH='/home/jenkins/.dotnet/tools'       
        SONAR_SERVER='http://sonarqube.orizon.local'
            ...
            stages {
                ...
                stage('clean') {
                    ...
                    steps 
                    {
                        ...
                        sh 'dotnet restore -s http://nexus.orizon.local/repository/nuget-group/ -s https://api.nuget.org/v3/index.json ${PRJ_PATH}/${PRJ_NAME}'
                    }
                }
                stage('sonar-config') {
                   steps 
                ...
                stage('Sonar-Start') {
                    steps
                    {
                        script 
                        {
                            withSonarQubeEnv('sonarqube orizon')  
                            {           
                                echo "QUEM FEZ COMMIT? ${env.gitlabUserName}"
                            ...
                            steps
                            {
                                script
                                {
                                    def handle = triggerRemoteJob(
                                        job: 'http://jenkins-ci.orizon.local/job/RunFortify',
                            ...
                            timeout(time:10, unit:'MINUTES') {
                               input message: "Promote TAG ${env.image_tag} to PRD?", ok: "Promote"
                                ...
                                sh """
                                    ...
                                    oc image mirror --insecure docker-registry-default.cpdh.orizon.local/${OPENSHIFT_PROJECT_NAME}-hml/${APPLICATION_NAME}:${env.image_tag} docker-registry-default.cppr.orizon.local/${OPENSHIFT_PROJECT_NAME}/${APPLICATION_NAME}:${env.image_tag}
                                """
    ...
    post {
        always  {
            script {
                // Para lista completa de cows, acesse: http://cowsay-fature-auditado-dev.cpdh.orizon.local/fun.pl?list
                ...
                def response = httpRequest "http://cowsay-fature-auditado-dev.cpdh.orizon.local/fun.pl?addmsg=${msg}&uso=chat&cows=${cows}"
